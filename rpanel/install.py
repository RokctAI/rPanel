...
def after_install():
    ...
    if os.environ.get("CI") or os.environ.get("NON_INTERACTIVE"):
        print(
            "CI/Non-Interactive environment detected: Skipping system dependency installs in after_install"
        )
    else:
...
def check_and_install_system_dependencies():  # noqa: C901
    ...
    for dep_name, dep_info in dependencies.items():
        try:
            ...
            result = subprocess.run(dep_info["check"], shell=False, capture_output=True)
...
def setup_security_features():
    ...
    try:
        ...
        result = subprocess.run("nginx -V 2>&1 | grep -q modsecurity", shell=False)  # nosec B602 — pipe required for version check
        pkg_result = subprocess.run(
            "dpkg -l | grep -q libnginx-mod-http-modsecurity", shell=False
        )  # nosec B602 — pipe required
