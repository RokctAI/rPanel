    ...
    def install_roundcube(self):
        ...
        try:
            ...
            subprocess.run(
                ...
                check=True)
            subprocess.run(
                ["sudo", "mysql", "-e", "FLUSH PRIVILEGES;"], check=True)
            ...
            subprocess.run(["sudo", "wget", download_url,
                           "-O", tmp_tar], check=True)
            subprocess.run(["sudo", "mkdir", "-p", tmp_extract], check=True)
            subprocess.run(["sudo", "tar", "-xzf", tmp_tar,
                           "-C", tmp_extract], check=True)

            # Move to /var/www/roundcube
            # The tar contains a folder like 'roundcubemail-1.6.9'
            extracted_folder = subprocess.check_output(
                ["ls", tmp_extract], text=True).strip()
            ...
            # Sync files
            subprocess.run(
                ["sudo", "rsync", "-a", f"{src}/", f"{target_dir}/"], check=True)

            # Cleanup
            subprocess.run(
                ["sudo", "rm", "-rf", tmp_extract, tmp_tar], check=True)
            ...
            try:
                with open(sql_path, 'r') as sql_file:
                    subprocess.run(["sudo", "mysql", db_name],
...
?>"""
            ...
            # 5. Permissions
            subprocess.run(
                ["sudo", "chown", "-R", "www-data:www-data", target_dir], check=True)
            # Logs and Temp need write
            subprocess.run(["sudo", "chmod", "-R", "775",
                           os.path.join(target_dir, "logs")], check=True)
            subprocess.run(["sudo", "chmod", "-R", "775",
                           os.path.join(target_dir, "temp")], check=True)
    ...
    def run_command(self, command, description):
        try:
            ...
            process = subprocess.run(
                ...
                check=True)
            frappe.msgprint(
                f"{description} Successful.<br><pre>{
                    process.stdout}</pre>")

        except subprocess.CalledProcessError as e:
            frappe.log_error(f"{description} Failed: {e.stderr}")
            frappe.throw(
                f"{description} Failed.<br>Error Code: {
                    e.returncode}<br><pre>{
                    e.stderr}</pre>")
...
def get_system_status():
    ...
    for service in services:
        try:
            ...
            status[service] = {'status': result.stdout.strip(
            ) if result.returncode == 0 else 'inactive'}
        except Exception:
