...
class VPSPoolManager(VPSProvider):
	...
	def __init__(self, provider_priority=None):
		...
		for p in priority:
			try:
				...
				frappe.log_error(f"Failed to load provider '{p}' into pool: {e}", "VPS Pool Init Error")
	...
	def create_vps(self, plan_code: str, site_name: str, **kwargs) -> dict:
		...
		for name, provider in self.pool:
			try:
				frappe.log(f"VPS Pool Manager: Attempting creation with provider '{name}' for {site_name}")
				res = provider.create_vps(plan_code, site_name, **kwargs)
				if res and res.get("status") == "success":
					res["provider"] = name
					frappe.log(f"VPS Pool Manager: SUCCESS with provider '{name}' for {site_name}")
					return res
				else:
				...
				frappe.log_error(f"VPS Pool Manager: Exception on provider '{name}': {e}", "VPS Pool Cascading Error")

		# If we reach here, all providers in the pool failed
		err_summary = " | ".join(errors)
		frappe.log_error(f"VPS Pool exhausted. All creations failed: {err_summary}", "VPS Pool Exhaustion")
		return {
			"status": "failed",
