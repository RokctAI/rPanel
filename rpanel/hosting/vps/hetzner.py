...
class HetznerVPSProvider(VPSProvider):
	...
	def create_vps(self, plan_code: str, site_name: str, **kwargs) -> dict:
		...
		try:
			...
			server_type = "cpx21" if plan_code in ["vps-1", "vps-comfort-8"] else plan_code
			
			# Use default location fsn1 (Falkenstein, Germany) or ash (Ashburn, USA) if CA/NA preferred
			location = kwargs.get("location") or frappe.conf.get("hetzner_location") or "fsn1"
			image = kwargs.get("image") or frappe.conf.get("hetzner_image") or "ubuntu-24.04"
			...
			if ssh_keys:
				payload["ssh_keys"] = ssh_keys if isinstance(ssh_keys, list) else [ssh_keys]

			response = requests.post(f"{self.api_url}/servers", json=payload, headers=self.headers, timeout=30)
			...
			if response.status_code == 201:
				...
				frappe.log(f"SUCCESS: Hetzner Server created for {site_name} (ID: {server.get('id')})")
				return {
					"status": "success",
			...
			frappe.log_error(f"Hetzner Server Creation Failed for {site_name}: {e}", "VPS Orchestrator Error")
	...
	def rebuild_vps(self, vps_id: str, image_name: str, ssh_keys: list, **kwargs) -> bool:
		...
		try:
			...
			response = requests.post(f"{self.api_url}/servers/{vps_id}/actions/rebuild", json=payload, headers=self.headers, timeout=30)
			return response.status_code == 201
		except Exception as e:
			frappe.log_error(f"Hetzner Rebuild Failed for {vps_id}: {e}", "VPS Orchestrator Error")
			return False
	...
	def get_vps_status(self, vps_id: str) -> dict:
		...
		try:
			response = requests.get(f"{self.api_url}/servers/{vps_id}", headers=self.headers, timeout=20)
			if response.status_code == 200:
				server = response.json().get("server", {})
				return {
					"vps_id": vps_id,
					"state": "running" if server.get("status") == "running" else "stopped",
					"ip": server.get("public_net", {}).get("ipv4", {}).get("ip"),
					"memory": server.get("server_type", {}).get("memory", 0) * 1024, # Convert GB to MB
					"vcpus": server.get("server_type", {}).get("cores", 0),
					"raw_info": server
				}
			return {"vps_id": vps_id, "state": "unknown", "error": response.text}
		except Exception as e:
			frappe.log_error(f"Hetzner Status Query Failed for {vps_id}: {e}", "VPS Orchestrator Error")
	...
	def reboot_vps(self, vps_id: str, hard: bool = False) -> bool:
		...
		try:
			...
			response = requests.post(f"{self.api_url}/servers/{vps_id}/actions/{action}", headers=self.headers, timeout=30)
			return response.status_code == 201
		except Exception as e:
			frappe.log_error(f"Hetzner Reboot Failed for {vps_id}: {e}", "VPS Orchestrator Error")
			return False
	...
	def terminate_vps(self, vps_id: str, **kwargs) -> bool:
		...
		try:
			response = requests.delete(f"{self.api_url}/servers/{vps_id}", headers=self.headers, timeout=30)
			return response.status_code == 200 or response.status_code == 202
		except Exception as e:
			frappe.log_error(f"Hetzner Server Deletion Failed for {vps_id}: {e}", "VPS Orchestrator Error")
			return False
