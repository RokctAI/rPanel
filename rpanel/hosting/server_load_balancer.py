...
def get_optimal_server_with_load_balancing(group='Production', algorithm='least_loaded'):  # noqa: C901
    ...
    if algorithm == 'round_robin':
        ...
        available_servers = [
            s for s in servers if s.current_websites < s.max_websites]
        if available_servers:
            # Calculate weight: (cpu_cores * ram_gb) / current_websites
            for server in available_servers:
                weight = (server.cpu_cores * server.ram_gb) / \
                    max(server.current_websites, 1)
...
def distribute_websites_across_group(group_name):
    ...
    return {
        ...
        'message': f'Distributed {distributed} websites across {
            len(servers)} servers'}
